...
export default function MatchRequestCard({ request, side, onAccept, onReject, busy }: Props) {
  ...
  const statusLabel =
    request.status === 'accepted' ? '매칭 성공' :
    request.status === 'rejected' ? (side === 'outgoing' ? '거절되었습니다' : '거절함') :
    request.status === 'cancelled' ? '자동 종료' :
    '대기 중';
  ...
  return (
    ...
    >
      ...
      <p className="mt-1 inline-flex items-center gap-1 text-xs text-zinc-500">
        {side === 'incoming'
          ? <><Inbox size={13} strokeWidth={1.8} />우리에게 신청을 보낸 팀</>
          : <><Send size={13} strokeWidth={1.8} />내가 신청을 보낸 팀</>}
      </p>
      ...
      {side === 'incoming' && request.status === 'pending' && (
        <div className="mt-4 grid grid-cols-2 gap-2" onClick={(e) => e.stopPropagation()}>
          <Button variant="ghost" disabled={busy} onClick={() => onReject?.(request.id)}>거절</Button>
          <Button className="gap-1.5" disabled={busy} onClick={() => onAccept?.(request.id)}>
            <Heart size={15} strokeWidth={2} />수락
          </Button>
        </div>
      )}
      ...
      {side === 'outgoing' && request.status === 'rejected' && (
        <p className="mt-3 text-sm text-zinc-500">
          아쉽지만 이번 인연은 아니었나봐요. 다른 팀에도 도전해보세요.
        </p>
      )}
      ...
      {request.status === 'cancelled' && (
        <p className="mt-3 rounded-2xl bg-sky-50 px-4 py-2.5 text-sm leading-relaxed text-sky-800 ring-1 ring-sky-100">
          {side === 'outgoing'
            ? '상대 팀이 다른 팀과 먼저 매칭되어 이 신청은 자동으로 종료되었어요. 거절당하신 게 아닙니다.'
            : '우리 팀이 다른 팀과 매칭되어 이 신청은 자동으로 종료되었어요.'}
        </p>
      )}
    </article>
  );
